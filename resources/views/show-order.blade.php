...
<body>
    ...
    <main>
        <section class="about section--nopb" style="padding-top: 140px;">
            <div class="container">
                <h1 class="text-center mt-2" style="font-size: 40px; font-weight: 800; color: #4682A9;">Pesanan Saya</h1>

                @if ($order->status == 'Belum Dibayar' || $order->status == 'Menunggu Konfirmasi Pembayaran')
                <div class="content d-flex flex-column flex-md-row my-5 gap-5">
                    <div class="col-md-5">
                        <p class="my-0 text-center" style="color: #556260;">Kode Pesanan: {{ $order->kode_pesanan }}</p>
                        <h3 class="heading text-center mt-0 mb-4">Rp {{ number_format($order->total_harga, 0, ',', '.') }}</h3>
                    ...
                    <div class="col-md-7">
                        ...
                        <form action="/orders/{{ $order->kode_pesanan }}" method="post" enctype="multipart/form-data" class="bukti_bayar my-0">
                            @method('put')
                            @csrf
                            <label for="bukti_pembayaran" class="form-label">Bukti Pembayaran</label>
                            <div class="row">
                                <div class="d-flex col-9 align-items-center">
                                    <input type="file" name="bukti_pembayaran" id="bukti_pembayaran" class="form-control m-auto @error('bukti_pembayaran') is-invalid @enderror">
                                </div>
                                <div class="col-3">
                        ...
                        <p class="text-danger mb-4">*Pesanan tidak dapat dibatalkan setelah pembayaran diverfikasi</p>

                        <div class="d-flex flex-column flex-md-row gap-2 gap-md-3">
                            <div class="cancel col-12 col-md-6 h-100 my-md-auto mb-3">
                                <form action="/orders/{{ $order->kode_pesanan }}" method="post" class="d-inline" style="background-color:#084e7e">
                                    @method('delete')
                                    @csrf
                                    <button type="submit" onclick="return confirm('Apakah kamu yakin akan membatalkan pesanan ini?')" id="outline-button" style="border-radius: 30px;">Batalkan Pesanan</button>
                                </form>
                            </div>
                ...
                <div class="content d-flex flex-column-reverse flex-md-row my-5 gap-5">
                        ...
                        <div class="d-flex flex-column gap-2">
                            <div class="col">
                                <p class="status {{ $order->status == 'Belum Dibayar' ? '' : 'active' }} px-3 py-2 my-2">
                                    ...
                                    Pesanan Selesai
                                    @else
                                    Pesanan Anda {{ $order->status }}
                                    @endif
                                </p>
                                <p class="info my-2">Kode Pesanan : {{ $order->kode_pesanan }}</p>
                                <p class="info mb-2">Waktu Pemesanan : {{ $order->created_at->format('H:i - d/m/Y') }}</p>
                            </div>
                    ...
                    <div class="col-md-5">
                        ...
                        <div class="status-container">
                            ...
                            <!-- Fase 3: Selesai -->
                            <div class="status-item">
                                <div class="circle {{ $order->status == 'Pesanan Selesai' ? 'active' : '' }}">
                ...
                <form action="" class="my-5">
                    ...
                    <div class="my-4 button d-flex justify-content-between">
                        ...
                        <div class="col-8 col-xl-3">
                            <a href="/orders" id="back-button" style="border-radius: 30px;">
                                <svg width="27" height="27" viewBox="0 0 27 27" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M19.0466 7.875C19.0824 7.79527 19.0991 7.70838 19.0956 7.62111H19.125C18.9992 4.61915 16.5205 2.25 13.5056 2.25C10.4906 2.25 8.01194 4.61915 7.88612 7.62111C7.87129 7.7051 7.87129 7.79102 7.88612 7.875H7.79818C6.3565 7.875 4.81539 8.82671 4.36797 11.3851L3.49302 18.3541C2.77715 23.4708 5.41195 24.75 8.8521 24.75H18.1783C21.6085 24.75 24.1637 22.8977 23.5274 18.3541L22.6624 11.3851C22.1354 8.89835 20.6441 7.875 19.2223 7.875H19.0466ZM17.4299 7.875C17.3986 7.79394 17.382 7.70797 17.3809 7.62111C17.3809 5.46393 15.6261 3.71518 13.4615 3.71518C11.2968 3.71518 9.54207 5.46393 9.54207 7.62111C9.5569 7.7051 9.5569 7.79102 9.54207 7.875H17.4299ZM10.2341 13.6671C9.68501 13.6671 9.23986 13.209 9.23986 12.6438C9.23986 12.0786 9.68501 11.6205 10.2341 11.6205C10.7832 11.6205 11.2284 12.0786 11.2284 12.6438C11.2284 13.209 10.7832 13.6671 10.2341 13.6671ZM15.7523 12.6438C15.7523 13.209 16.1974 13.6671 16.7465 13.6671C17.2957 13.6671 17.7408 13.209 17.7408 12.6438C17.7408 12.0786 17.2957 11.6205 16.7465 11.6205C16.1974 11.6205 15.7523 12.0786 15.7523 12.6438Z" fill="currentColor" />
